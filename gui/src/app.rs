...
/// Lays out `add` inside a horizontally centered column of at most `max_w`
/// points, so pages read like a document instead of hugging the left edge of
/// a maximized window.
fn centered_column(ui: &mut egui::Ui, max_w: f32, add: impl FnOnce(&mut egui::Ui)) {
...
impl DashboardApp {
    ...
    fn draw_tools_page(&mut self, ui: &mut egui::Ui) {
        ...
        centered_column(ui, 720.0, |ui| {
            ...
            match action {
                Some(Action::Sdio) => match self.resolve_sdio_exe() {
                    Some(exe) => {
                    ...
                    None => {
                        self.push_error("SDIO executable not found. Set SDIOPath in settings.json.")
                    }
                },
                Some(Action::Rapr) => {
    ...
    fn draw_install_page(&mut self, ui: &mut egui::Ui) {
        ...
        centered_column(ui, 860.0, |ui| {
            ...
            if !self.bundle_apps.is_empty() {
                ...
                theme::card().show(ui, |ui| {
                    // Two columns per category: name + visible description,
                    // with full details (ids, category) on hover.
                    const COLS: usize = 2;
        ...
        .show(ui, |ui| {
            ...
            ui.add_enabled_ui(enabled, |ui| {
                ui.horizontal_top(|ui| {
                    ...
                    // column has the same width in every card.
                    let chip_w = 90.0;
                    ui.vertical(|ui| {
