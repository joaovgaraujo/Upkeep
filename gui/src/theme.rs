...
/// Windows' own "Make text bigger" setting (Settings > Accessibility > Text
/// size), as a zoom-compatible multiplier (125% -> 1.25). Monitor/display
/// scaling (100%/125%/150%...) is a *separate* per-monitor DPI setting that
/// eframe/winit already apply automatically via `native_pixels_per_point`
/// from the `PerMonitorV2` manifest in build.rs -- this only covers the
/// additional accessibility text-size slider, which nothing here reads
/// automatically otherwise.
///
/// Used only to pick a sensible *default* on first run, before
/// `settings.json` exists: once the user has an explicit `UiScale`, that
/// value always wins, even if it happens to equal this function's result.
/// Falls back to 1.0 (no extra scaling) if the registry value is absent,
/// which is the normal case on a machine that has never touched that slider.
pub fn windows_text_scale() -> f32 {
    use winreg::enums::HKEY_CURRENT_USER;
    use winreg::RegKey;
    RegKey::predef(HKEY_CURRENT_USER)
        .open_subkey(r"SOFTWARE\Microsoft\Accessibility")
        .and_then(|key| key.get_value::<u32, _>("TextScaleFactor"))
        .map(|pct| clamp_scale(pct as f32 / 100.0))
        .unwrap_or(1.0)
}
