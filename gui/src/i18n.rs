...
pub struct Strings {
    ...
    pub drivers_update_btn: &'static str,
    pub drivers_update_hint: &'static str,
...
pub static EN: Strings = Strings {
    tagline: "Keep your PC updated and set up new ones in a few clicks.",
    ...
    run_cta: "Update checked categories",
    stop_cta: "Stop this run",
    stopping_button: "Stopping...",
    view_full_log: "View full log",
    result_ok_title: "\u{2714} All done: everything is up to date",
    ...
    install_subtitle: "Tick any apps you want and press Install, or pick a starter pack to pre-select a good set.",
    ...
    pack_full_desc: "The complete catalog: pick and choose from all available apps.",
    pack_generic_desc: "A custom app pack.",
    pack_apps_suffix: "apps",

    setup_title: "Set up a new PC",
    setup_subtitle: "For a freshly installed Windows. Tick the steps you want and press the button. A window opens showing progress, Windows asks for permission once (UAC), and a restart at the end is recommended.",
    ...
    oosu_mode_auto: "Automatic: recommended settings, no clicks needed",
    oosu_mode_manual: "Manual: opens the program for you to choose",
    ...
    tools_nvidia_auto: "NVIDIA driver: automatic clean update",
    ...
    summary_empty: "No summary yet. Run a category first.",
    ...
    tools_nvclean_recommended: "NVCleanstall: Recommended Update",
    ...
    system_subtitle: "What runs automatically on your PC, and the switch to turn each thing off. Hover any item for an explanation of what it is.",
    ...
    startup_toggle_hover: "Same switch as Task Manager's Startup tab. Nothing is deleted, and you can turn it back on any time.",
    advice_col: "Advice",
    advice_keep: "keep",
    ...
    tasks_intro: "Scheduled tasks that fire at boot or sign-in, mostly updaters and helpers apps register for themselves. The switch uses Windows' own enable/disable for tasks; nothing is deleted.",
    ...
    system_export_hover: "Writes StartupReport.md with every startup program, scheduled task and service, including the file each one launches, so you can research items one by one.",
    system_advanced_label: "Advanced",
    system_advanced_hover: "Also show the items best left alone: everything marked 'keep', and (for services) the ones this tool doesn't recognize, mostly core Windows components.",
    services_intro: "Background services Windows runs. Hover a service to read Windows' own description of what it does. The start-type menu changes how it launches. 'Disabled' stops it completely, so only disable things you recognize.",
    ...
    startup_intro: "Programs that launch automatically with Windows. Use the On/Off switch to stop one. It uses the same mechanism as Task Manager, deletes nothing, and is reversible. Hover a name to see where it comes from.",
    ...
    drivers_intro: "Windows keeps every driver version ever installed in its 'driver store'. Old versions (GPU drivers especially) can waste gigabytes. This lists them; to clean up safely use Driver Store Explorer on the Tools page.",

    drivers_refresh: "Refresh",
    drivers_update_btn: "Update drivers (SDIO)\u{2026}",
    drivers_update_hint: "Opens Snappy Driver Installer Origin so you can review what it proposes. A bad driver can stop a PC booting, so nothing is installed automatically. SDIO makes a restore point first. Its first run downloads driver packs (potentially several GB, over BitTorrent). Prefer its \"missing\" and \"better\" matches. For NVIDIA, the Tools page's NVCleanstall path is the better route.",
...
pub static PT_BR: Strings = Strings {
    tagline: "Mantenha seu PC atualizado e configure PCs novos em poucos cliques.",
    ...
    run_cta: "Atualizar categorias marcadas",
    stop_cta: "Parar esta execu\u{e7}\u{e3}o",
    stopping_button: "Parando...",
    view_full_log: "Ver log completo",
    result_ok_title: "\u{2714} Tudo pronto: seu PC est\u{e1} atualizado",
    ...
    install_subtitle: "Marque os apps que quiser e aperte Instalar, ou escolha um pacote inicial para pr\u{e9}-selecionar um bom conjunto.",
    ...
    pack_full_name: "Tudo",
    pack_full_desc: "O cat\u{e1}logo completo: escolha entre todos os apps dispon\u{ed}veis.",
    pack_generic_desc: "Um pacote de apps personalizado.",
    pack_apps_suffix: "apps",

    setup_title: "Configurar um PC novo",
    setup_subtitle: "Para um Windows rec\u{e9}m-instalado. Marque as etapas desejadas e aperte o bot\u{e3}o. Uma janela mostra o progresso, o Windows pede permiss\u{e3}o uma vez (UAC) e, ao final, \u{e9} recomendado reiniciar o PC.",
    ...
    oosu_mode_auto: "Autom\u{e1}tico: configura\u{e7}\u{f5}es recomendadas, sem cliques",
    oosu_mode_manual: "Manual: abre o programa para voc\u{ea} escolher",
    ...
    tools_nvidia_auto: "Driver NVIDIA: atualiza\u{e7}\u{e3}o limpa autom\u{e1}tica",
    ...
    summary_empty: "Nenhum resumo ainda. Execute uma categoria primeiro.",
    ...
    tools_nvclean_recommended: "NVCleanstall: Atualiza\u{e7}\u{e3}o Recomendada",
    ...
    system_subtitle: "O que roda automaticamente no seu PC, e o interruptor para desligar cada item. Passe o mouse sobre um item para ver a explica\u{e7}\u{e3}o do que ele \u{e9}.",
    ...
    startup_toggle_hover: "Mesmo interruptor da aba Inicializar do Gerenciador de Tarefas. Nada \u{e9} apagado e d\u{e1} para religar quando quiser.",
    ...
    tasks_intro: "Tarefas agendadas que disparam no boot ou no login, em geral atualizadores e auxiliares que os pr\u{f3}prios apps registram. O interruptor usa o ativar/desativar do pr\u{f3}prio Windows; nada \u{e9} apagado.",
    ...
    system_export_hover: "Gera o StartupReport.md com todos os programas de inicializa\u{e7}\u{e3}o, tarefas agendadas e servi\u{e7}os, incluindo o arquivo que cada um executa, para voc\u{ea} pesquisar item por item.",
    system_advanced_label: "Avan\u{e7}ado",
    system_advanced_hover: "Mostra tamb\u{e9}m os itens que \u{e9} melhor n\u{e3}o mexer: tudo marcado como 'manter' e (nos servi\u{e7}os) os que a ferramenta n\u{e3}o reconhece, em geral componentes essenciais do Windows.",
    services_intro: "Servi\u{e7}os que o Windows executa em segundo plano. Passe o mouse sobre um servi\u{e7}o para ler a descri\u{e7}\u{e3}o do pr\u{f3}prio Windows. O menu de tipo de in\u{ed}cio muda como ele inicia. 'Disabled' para o servi\u{e7}o por completo, ent\u{e3}o desative s\u{f3} o que voc\u{ea} reconhece.",
    ...
    startup_intro: "Programas que iniciam automaticamente com o Windows. Use o interruptor Liga/Desliga para impedir um deles. \u{c9} o mesmo mecanismo do Gerenciador de Tarefas, nada \u{e9} apagado e d\u{e1} para reverter. Passe o mouse sobre o nome para ver de onde ele vem.",
    ...
    drivers_intro: "O Windows guarda todas as vers\u{f5}es de driver j\u{e1} instaladas no 'reposit\u{f3}rio de drivers'. Vers\u{f5}es antigas (principalmente de v\u{ed}deo) podem desperdi\u{e7}ar gigabytes. Esta lista mostra todas; para limpar com seguran\u{e7}a, use o Driver Store Explorer na p\u{e1}gina Ferramentas.",

    drivers_refresh: "Atualizar",
    drivers_update_btn: "Atualizar drivers (SDIO)\u{2026}",
    drivers_update_hint: "Abre o Snappy Driver Installer Origin para voc\u{ea} conferir o que ele prop\u{f5}e. Um driver ruim pode impedir o PC de iniciar, ent\u{e3}o nada \u{e9} instalado sozinho. O SDIO cria um ponto de restaura\u{e7}\u{e3}o antes. Na primeira execu\u{e7}\u{e3}o ele baixa pacotes de drivers (podem ser v\u{e1}rios GB, por BitTorrent). Prefira as op\u{e7}\u{f5}es \"missing\" e \"better\". Para NVIDIA, o caminho do NVCleanstall na p\u{e1}gina Ferramentas \u{e9} melhor.",
...
pub fn startup_error(lang: Lang, bat_name: &str) -> String {
    match lang {
        Lang::En => format!(
            "Could not locate {bat_name}. Searched upward from the executable's folder and the current directory. Running the engine is disabled until this is resolved."
        ),
        Lang::PtBr => format!(
            "N\u{e3}o foi poss\u{ed}vel localizar {bat_name}. Busca realizada a partir da pasta do execut\u{e1}vel e do diret\u{f3}rio atual, subindo os n\u{ed}veis. A execu\u{e7}\u{e3}o do motor est\u{e1} desabilitada at\u{e9} que isso seja resolvido."
        ),
    }
}
...
pub fn tweak_text(lang: Lang, slug: &str) -> (&'static str, &'static str) {
    match lang {
        ...
        Lang::PtBr => match slug {
            ...
            "explorer-discovery" => ("Pastas que abrem mais r\u{e1}pido", "O Windows para de analisar o conte\u{fa}do de cada pasta para \u{ab}adivinhar\u{bb} o layout, ent\u{e3}o as pastas abrem na hora."),
...
pub fn toggle_text(lang: Lang, slug: &str) -> (&'static str, &'static str) {
    match lang {
        Lang::En => match slug {
            ...
            "file-extensions" => ("Show file extensions", "Shows the .pdf/.exe ending of file names, which helps you spot fake files."),
            "hidden-files" => ("Show hidden files", "Shows files Windows normally hides."),
            "mouse-accel-off" => ("Mouse acceleration off", "The cursor moves the same distance no matter how fast you move the mouse: more precise, great for games."),
        ...
        Lang::PtBr => match slug {
            ...
            "file-extensions" => ("Mostrar as extens\u{f5}es dos arquivos", "Mostra a termina\u{e7}\u{e3}o .pdf/.exe nos nomes de arquivo, o que ajuda a reconhecer arquivos falsos."),
            "hidden-files" => ("Mostrar arquivos ocultos", "Exibe os arquivos que o Windows normalmente esconde."),
            "mouse-accel-off" => ("Desligar a acelera\u{e7}\u{e3}o do mouse", "O cursor percorre sempre a mesma dist\u{e2}ncia, n\u{e3}o importa a velocidade do movimento: mais precis\u{e3}o, \u{f3}timo para jogos."),
