...
- **Pins.** A pinned package stops receiving updates, *including security
  updates*, so nothing security-relevant ships pinned. The only two defaults
  are MiKTeX and Heroic, where the updater itself is broken and pinning merely
  removes a guaranteed failure from every run — both are commented at the
  `[pins]` section. Add your own from the Pins tab.
