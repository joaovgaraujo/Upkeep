...
rem    A pin stops a package receiving updates INCLUDING SECURITY UPDATES.
rem    Two different reasons appear below, and the difference matters:
rem
rem      * Adobe Acrobat Reader is pinned BY CHOICE. Acrobat is a browser-
rem        adjacent PDF renderer and a regular target for exploited CVEs, so
rem        pinning it means running knowingly-outdated software until you
rem        unpin or update it another way. This is the project author's
rem        preference; if you are not them, decide for yourself. Remove these
rem        lines, or manage them from the GUI's Pins tab.
rem          winget pin remove --id Adobe.Acrobat.Reader.64-bit
rem          choco  pin remove -n=adobereader
rem
rem      * MiKTeX and Heroic are pinned because their UPDATERS ARE BROKEN --
rem        pinning removes a guaranteed per-run failure and costs nothing.
rem
...
where winget >nul 2>&1 && (
    rem Preference pins - see the security note above.
    winget pin add --id Adobe.Acrobat.Reader.64-bit --accept-source-agreements >nul 2>&1
    winget pin add --id Adobe.Acrobat.Reader.32-bit --accept-source-agreements >nul 2>&1
    winget pin add --id Adobe.Acrobat.Pro          --accept-source-agreements >nul 2>&1
...
where choco >nul 2>&1 && (
    ...
    choco pin add -n=adobe-acrobat-reader-dc  >nul 2>&1
    choco pin add -n=adobeair                 >nul 2>&1
)
...
rem ============================================================
rem
rem  Excluding a package is done with package manager pins, not regex. Adobe
rem  Acrobat IS pinned by default (a deliberate choice - see the [pins] section
rem  for why that is a security trade-off). Manage pins from the Pins tab or:
rem    winget pin list
